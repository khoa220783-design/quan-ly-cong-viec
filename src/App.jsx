...
import Dashboard from './modules/task/components/Dashboard';
...
// MainContent component - shows LandingPage or TaskList/Dashboard based on wallet connection and active tab
const MainContent = ({ activeTab }) => {
  ...
  return activeTab === 'dashboard' ? <Dashboard /> : <TaskList />;
};

// App Layout component - inside provider tree so it can use wallet context
const AppLayout = ({ isScrolled, activeTab, setActiveTab }) => {
  const { diaChiVi } = useWalletContext();
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center gap-4">
              ...
              {diaChiVi && (
                <nav className="hidden md:flex items-center gap-1 ml-8 bg-surface border border-border rounded-lg p-1">
                  <button
                    onClick={() => setActiveTab('tasks')}
                    className={`px-4 py-1.5 rounded-md font-mono text-xs font-medium transition-all ${activeTab === 'tasks'
                  ...
                  <button
                    ...
                    className={`px-4 py-1.5 rounded-md font-mono text-xs font-medium transition-all ${activeTab === 'dashboard'
                        ? 'bg-neon-green/10 text-neon-green'
                        : 'text-secondary hover:text-primary'
                      }`}
                  >
...
function AppContent() {
  ...
  return (
    <WalletProvider>
      <ContractProvider>
        <TaskProvider>
          <AppLayout isScrolled={isScrolled} activeTab={activeTab} setActiveTab={setActiveTab} />
        </TaskProvider>
      </ContractProvider>
    </WalletProvider>
  );
